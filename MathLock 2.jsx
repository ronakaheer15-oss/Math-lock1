...
// ─── AI ANSWER CHECKER ────────────────────────────────────────────────────
async function checkAnswerWithAI(question, userAnswer, chapter) {
...
export default function MathLock() {
  ...
  function toggleTask(di,ti){
    setDoneTasks(prev=>{
      ...
      if(arr.length===ROADMAP[di].tasks.length)setTimeout(()=>{
  ...
  function addMistake(){
    ...
    setMistakes(prev=>[{id:Date.now(),text:newMistake,chapter:mistakeChap||day.chapter,day:currentDay+1,date:new Date().toLocaleDateString()},{...prev[0],...prev.slice(1)},...prev.slice(1)].slice(0,50));
  ...
  async function runChecker(){
    ...
    try{
      const res=await checkAnswerWithAI(checkerQ,checkerA,day.chapter);
      setCheckerRes(res);
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"system-ui,sans-serif",padding:14,maxWidth:540,margin:"0 auto",paddingBottom:40}}>
      ...
      {activeTab==="checker"&&(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:16}}>
          ...
          <div style={{fontSize:11,color:C.muted,marginBottom:16}}>Write any question + your answer. AI will check it and give feedback.</div>
